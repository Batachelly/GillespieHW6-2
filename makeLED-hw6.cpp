...
int main(int argc, char* argv[]){
   if(argc!=3){
	cout << "Usage is makeLED-hw6 followed by one of: " << endl;
        cout << "   on, off, flash, blink, or status" << endl;
	cout << " followed by a number" << endl;
	cout << " The number is used only by blink and ignored by the others." << endl;
	cout << " e.g. makeLED-hw6 flash 0" << endl;
	cout << " e.g. makeLED-hw6 on 2" << endl;
	cout << " e.g. makeLED-hw6 blink 3" << endl;
        return 2;
   }
   string cmd(argv[1]);
   ...
   if(cmd=="on"){
	...
	removeTrigger();
	   ...
	   // stoi() converts char array or string to int.
	   // atoi() is a legacy C-style function, which works too
	   // but requires <stdlib.h>
	   int count=stoi(argv[2]);
	   cout << "Blinking the LED " << count << " times." << endl;
	   removeTrigger();
	   while(count > 0) {
		   writeLED("/brightness", "1");
		   sleep(1);
